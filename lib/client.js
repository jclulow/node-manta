...
MantaClient.prototype.medusaAttach = function medusaAttach(j, opts, cb) {
         ...
         * Phase functions:
         */
        var do_sign_request = function do_sign_request(_, next) {
                signRequest({
                        headers: options.headers,
                        sign: self.sign
                }, next);
        ...
        var do_websocket_get = function do_websocket_get(_, next) {
                self.client.get(options, function onRequest(err, req) {
                        ...
                        req.once('result', function(err, res) {
                                 ...
                                 */
                                next(new Error('Server did not Upgrade'));
                        });
                        req.once('upgradeResult', function(err, _r, _s, _h) {
                                if (fired) {
                                        _s.destroy();
                                        return;
                                }
        ...
        var do_websocket_attach = function do_websocket_attach(_, next) {
                /*
